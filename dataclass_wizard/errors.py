...
from typing import Any, Type, Dict, Tuple, ClassVar, Optional, Union, Iterable
...
class ParseError(JSONWizardError):
    ...
    _TEMPLATE = (
        "Failure parsing field `{field}` in class `{cls}`. Expected "
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._default_class_name = self.name(_default_class) if _default_class else None
...
class MissingFields(JSONWizardError):
    ...
    _TEMPLATE = (
        "Failure calling constructor method of class `{cls}`. "
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if next(
            (f for f in self.missing_fields if normalize(f) in normalized_json_keys),
            None,
        ):
            ...
            kwargs["resolution"] = (
                ...
                " https://github.com/rnag/dataclass-wizard/issues/54"
            )
...
class UnknownJSONKey(JSONWizardError):
    ...
    _TEMPLATE = (
        "A JSON key is missing from the dataclass schema for class `{cls}`.\n"
