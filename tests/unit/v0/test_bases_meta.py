...
from dataclass_wizard.v0.bases import META
from dataclass_wizard.v0 import JSONWizard, EnvWizard
from dataclass_wizard.v0.bases_meta import BaseJSONWizardMeta
from dataclass_wizard.v0.enums import LetterCase, DateTimeTo
from dataclass_wizard.v0.errors import ParseError
from dataclass_wizard.v0.utils.type_conv import date_to_timestamp
...
def mock_meta_initializers(mocker: MockerFixture):
    return mocker.patch('dataclass_wizard.v0.bases_meta.META_INITIALIZER')


@pytest.fixture
def mock_bind_to(mocker: MockerFixture):
    return mocker.patch(
        'dataclass_wizard.v0.bases_meta.BaseJSONWizardMeta.bind_to')


@pytest.fixture
def mock_env_bind_to(mocker: MockerFixture):
    return mocker.patch(
        'dataclass_wizard.v0.bases_meta.BaseEnvWizardMeta.bind_to')


@pytest.fixture
def mock_get_dumper(mocker: MockerFixture):
    return mocker.patch('dataclass_wizard.v0.bases_meta.get_dumper')
